...
input int EntryBars = 20;
input int ExitBars = 10;
...
void OnTick()
{
	...
	if(minuteBar == true)
   {
   	...
   	double openBuyPrice = HighestHigh(_Symbol, _Period, EntryBars);
   	double openSellPrice = LowestLow(_Symbol, _Period, EntryBars);
   	
   	double buyStopLoss = LowestLow(_Symbol, _Period, ExitBars);
   	double sellStopLoss = HighestHigh(_Symbol, _Period, ExitBars);
   		
   	tradeExist = CheckExistTrade();
   	
   	if(tradeExist == true)  DeletePending();
      
      int positions = 0, pending = 0;
      positions = Positions.TotalPositions(MagicNumber);
      pending = Pending.TotalPending(_Symbol);
      
      if(PositionSelect(_Symbol) == false && pending == 0)
         orderPlaced = false;
   	...
   	if(PositionSelect(_Symbol) == true)
   	{
   	   ...
   	   {
   	      glBuyTicket = PositionGetInteger(POSITION_TICKET);
   	      Trade.ModifyPosition(glBuyTicket, buyStopLoss);
   	   }
   	   if(positionType == POSITION_TYPE_SELL)
   	   {
   	       glSellTicket = PositionGetInteger(POSITION_TICKET);
   	       Trade.ModifyPosition(glSellTicket, sellStopLoss);
   	   }
   	}
   	
   	if(glBuyTicket > 0 && glSellTicket > 0) orderPlaced == true;
   	else orderPlaced == false;
